...
for (let y = 0; y < words.length; y++) {
    for (let x = 0; x < words[0].length; x++) {
        ...
        if (safeAccess(words, y, x + 3) + safeAccess(words, y, x + 2) + safeAccess(words, y, x + 1) + safeAccess(words, y, x) === 'XMAS') {
            xmas++
        }
        ...
        // vertical - backwards
        if (safeAccess(words, y + 3, x) + safeAccess(words, y + 2, x) + safeAccess(words, y + 1, x) + safeAccess(words, y, x) === 'XMAS') {
            xmas++
        }
        ...
        // diagonal - backwards
        if (safeAccess(words, y + 3, x + 3) + safeAccess(words, y + 2, x + 2) + safeAccess(words, y + 1, x + 1) + safeAccess(words, y, x) === 'XMAS') {
            xmas++
        }
